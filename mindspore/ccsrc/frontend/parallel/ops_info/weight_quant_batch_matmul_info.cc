...
constexpr char TRANSPOSE_X[] = "transpose_x";
constexpr char TRANSPOSE_WEIGHT[] = "transpose_weight";
}  // namespace
...
Status WeightQuantBatchMatmulInfo::GetAttrs() {
  ...
  if ((inputs_shape_.size() != kInputNum) || (outputs_shape_.size() != MATMUL_OUTPUTS_SIZE)) {
  ...
  if (mat_x_dimension_ < 2 || mat_weight_dimension_ < 2) {
...
Status WeightQuantBatchMatmulInfo::CheckInputStrategy(const Shape &mat_x_strategy, const Shape &mat_weight_strategy) {
  ...
  if (transpose_x_) {
    if (!transpose_weight_ &&
        (mat_x_strategy.at(SECOND_FROM_END(mat_x_size)) != mat_weight_strategy.at(SECOND_FROM_END(mat_weight_size)))) {
      // for example: mat_x_strategy:[2,4,16,8], mat_weight_strategy:[4,16,32], [16] in the example
      MS_LOG(ERROR) << name_ << ": Invalid strategy for mat_x " << ShapeToString(mat_x_strategy) << " and mat_weight "
                    << ShapeToString(mat_weight_strategy) << ". The transpose_x is: " << transpose_x_
                    << ", and transpose_weight is " << transpose_weight_ << ", the shard num of first input's row is "
                    << mat_x_strategy.at(SECOND_FROM_END(mat_x_size)) << ", but the shard num of second input's row is "
                    << mat_weight_strategy.at(SECOND_FROM_END(mat_weight_size));
      return FAILED;
    } else if (transpose_weight_ && (mat_x_strategy.at(SECOND_FROM_END(mat_x_size)) != mat_weight_strategy.back())) {
      // for example: mat_x_strategy:[2,4,16,8], mat_weight_strategy:[4,32,16], [16] in the example
      MS_LOG(ERROR) << name_ << ": Invalid strategy for mat_x " << ShapeToString(mat_x_strategy) << " and mat_weight "
                    << ShapeToString(mat_weight_strategy) << ". The transpose_x is: " << transpose_x_
                    << ", and transpose_weight is " << transpose_weight_ << ", the shard num of first input's row is "
                    << mat_x_strategy.at(SECOND_FROM_END(mat_x_size))
                    << ", but the shard num of second input's column is " << mat_weight_strategy.back();
      return FAILED;
    }
  } else {
    if (!transpose_weight_ && (mat_x_strategy.back() != mat_weight_strategy.at(SECOND_FROM_END(mat_weight_size)))) {
      // for example: mat_x_strategy:[2,4,8,16], mat_weight_strategy:[4,16,32], [16] in the example
      MS_LOG(ERROR) << name_ << ": Invalid strategy for mat_x " << ShapeToString(mat_x_strategy) << " and mat_weight "
                    << ShapeToString(mat_weight_strategy) << ". The transpose_x is: " << transpose_x_
                    << ", and transpose_weight is " << transpose_weight_
                    << ", the shard num of first input's column is " << mat_x_strategy.back()
...
Status WeightQuantBatchMatmulInfo::InferOutputTensorMap() {
  ...
  Shape mat_x_batch_map(mat_x_map.cbegin(), mat_x_map.cbegin() + mat_x_map.size() - 2);
  Shape mat_weight_batch_map(mat_weight_map.cbegin(), mat_weight_map.cbegin() + mat_weight_map.size() - 2);
  ...
  output_map.push_back(mat_x_map[mat_x_map.size() - 2]);
...
Status WeightQuantBatchMatmulInfo::InferTensorMap() {
  ...
  TensorMap antiquant_scale_tensor_map = {0};
  TensorMap antiquant_offset_tensor_map = {0};
  TensorMap quant_scale_tensor_map = {0};
  TensorMap quant_offset_tensor_map = {0};
  TensorMap bias_tensor_map = {0};

  inputs_tensor_map_.push_back(antiquant_scale_tensor_map);
  inputs_tensor_map_.push_back(antiquant_offset_tensor_map);
  inputs_tensor_map_.push_back(quant_scale_tensor_map);
  inputs_tensor_map_.push_back(quant_offset_tensor_map);
  inputs_tensor_map_.push_back(bias_tensor_map);
...
Status WeightQuantBatchMatmulInfo::InferTensorLayout(TensorLayouts *inputs_layout, TensorLayouts *outputs_layout) {
  ...
  TensorLayout mat_x_layout;
  TensorLayout mat_weight_layout;
  TensorLayout antiquant_scale_layout;
  TensorLayout antiquant_offset_layout;
  TensorLayout quant_scale_layout;
  TensorLayout quant_offset_layout;
  TensorLayout bias_layout;
  TensorLayout output_layout;
  if ((mat_x_layout.InitFromVector(dev_matrix_shape_, inputs_tensor_map_[kInputX], inputs_shape_[kInputX]) !=
       SUCCESS) ||
      (mat_weight_layout.InitFromVector(dev_matrix_shape_, inputs_tensor_map_[kInputWeight],
                                        inputs_shape_[kInputWeight]) != SUCCESS) ||
      (antiquant_scale_layout.InitFromVector(dev_matrix_shape_, inputs_tensor_map_[kInputAntiquantScale],
                                             inputs_shape_[kInputAntiquantScale]) != SUCCESS) ||
      (antiquant_offset_layout.InitFromVector(dev_matrix_shape_, inputs_tensor_map_[kInputQuantOffset],
                                              inputs_shape_[kInputQuantOffset]) != SUCCESS) ||
      (quant_scale_layout.InitFromVector(dev_matrix_shape_, inputs_tensor_map_[kInputQuantScale],
                                         inputs_shape_[kInputQuantScale]) != SUCCESS) ||
      (quant_offset_layout.InitFromVector(dev_matrix_shape_, inputs_tensor_map_[kInputAntiquantOffset],
                                          inputs_shape_[kInputAntiquantOffset]) != SUCCESS) ||
      (bias_layout.InitFromVector(dev_matrix_shape_, inputs_tensor_map_[kInputBias], inputs_shape_[kInputBias]) !=
       SUCCESS) ||
      (output_layout.InitFromVector(out_dev_matrix_shape_, outputs_tensor_map_[kOutput], outputs_shape_[kOutput]) !=
       SUCCESS)) {
    return FAILED;
  }

  if (field_size_ != 0) {
    mat_weight_layout.set_field_size(field_size_);
  }

  inputs_layout->push_back(mat_x_layout);
  inputs_layout->push_back(mat_weight_layout);
  inputs_layout->push_back(antiquant_scale_layout);
  inputs_layout->push_back(antiquant_offset_layout);
  inputs_layout->push_back(quant_scale_layout);
  inputs_layout->push_back(quant_offset_layout);
  inputs_layout->push_back(bias_layout);
...
Status WeightQuantBatchMatmulInfo::InferTensorInfo() {
  ...
  TensorLayout mat_x_layout = inputs_layout.at(kInputX);
  TensorLayout mat_weight_layout = inputs_layout.at(kInputWeight);
  TensorLayout antiquant_scale_layout = inputs_layout.at(kInputAntiquantScale);
  TensorLayout antiquant_offset_layout = inputs_layout.at(kInputAntiquantOffset);
  TensorLayout quant_scale_layout = inputs_layout.at(kInputQuantScale);
  TensorLayout quant_offset_layout = inputs_layout.at(kInputQuantOffset);
  TensorLayout bias_layout = inputs_layout.at(kInputBias);
  TensorLayout output_layout = outputs_layout.at(kOutput);
  TensorInfo mat_x_tensor_info(mat_x_layout);
  TensorInfo mat_weight_tensor_info(mat_weight_layout);
  TensorInfo antiquant_scale_layout_tensor_info(antiquant_scale_layout);
  TensorInfo antiquant_offset_layout_tensor_info(antiquant_offset_layout);
  TensorInfo quant_scale_layout_tensor_info(quant_scale_layout);
  TensorInfo quant_offset_layout_tensor_info(quant_offset_layout);
  TensorInfo bias_layout_tensor_info(bias_layout);
  TensorInfo output_tensor_info(output_layout);

  inputs_tensor_info_.push_back(mat_x_tensor_info);
  inputs_tensor_info_.push_back(mat_weight_tensor_info);
  inputs_tensor_info_.push_back(antiquant_scale_layout_tensor_info);
  inputs_tensor_info_.push_back(antiquant_offset_layout_tensor_info);
  inputs_tensor_info_.push_back(quant_scale_layout_tensor_info);
  inputs_tensor_info_.push_back(quant_offset_layout_tensor_info);
  inputs_tensor_info_.push_back(bias_layout_tensor_info);
...
std::vector<StrategyPtr> WeightQuantBatchMatmulInfo::GenerateOpStrategies(int64_t stage_id) {
  ...
  for (auto &sp : sp_vector) {
    ...
    Dimensions mat_x_strategy = tmp_strategy;
    mat_x_strategy.pop_back();  // [A, B, C, D]
    if (mat_x_shape.size() < mat_weight_shape.size()) {
      diff_len = mat_weight_shape.size() - mat_x_shape.size();
      (void)mat_x_strategy.erase(mat_x_strategy.cbegin(),
                                 mat_x_strategy.cbegin() + static_cast<different_type>(diff_len));  // [B, C, D]
    }

    // transpose_x
    if (transpose_x_) {
      (void)SwapLastTwoElements(&mat_x_strategy);
    }

    if (mat_x_strategy.size() != mat_x_shape.size()) {
      MS_LOG(EXCEPTION) << name_ << ": The size of mat_x_shape and mat_x_strategy must be equal, the mat_x_shape is "
                        << mat_x_shape << ", but the mat_x_strategy is " << mat_x_strategy;
    }

    // broadcast
    for (size_t i = 0; i < mat_x_strategy.size(); ++i) {
      if (mat_x_shape[i] <= 1) {
        mat_x_strategy[i] = NO_SPLIT_STRATEGY;
      }
    }

    // handle mat_weight's strategy
    Dimensions mat_weight_strategy = tmp_strategy;  // [A, B, C, D, E]
    // mat_weight_strategy: delete C, [A, B, D, E]
    (void)mat_weight_strategy.erase(mat_weight_strategy.cend() - 3);
    // mat_weight_strategy: delete A, [B, D, E]

    if (mat_weight_shape.size() < mat_x_shape.size()) {
      diff_len = mat_x_shape.size() - mat_weight_shape.size();
      (void)mat_weight_strategy.erase(mat_weight_strategy.cbegin(),
                                      mat_weight_strategy.cbegin() + static_cast<different_type>(diff_len));
    }

    // handle transpose_weight
    if (transpose_weight_) {
      (void)SwapLastTwoElements(&mat_weight_strategy);
    }

    // broadcast
    for (size_t i = 0; i < mat_weight_strategy.size(); ++i) {
      if (mat_weight_shape[i] <= 1) {
        mat_weight_strategy[i] = NO_SPLIT_STRATEGY;
      }
    }

    replace_strategy.push_back(mat_x_strategy);
    replace_strategy.push_back(mat_weight_strategy);
...
std::shared_ptr<Strategies> WeightQuantBatchMatmulInfo::GenerateBatchStrategies() {
  ...
  Dimensions batch_strategy_antiquant_scale(inputs_shape_[kInputAntiquantScale].size(), 1);
  Dimensions batch_strategy_antiquant_offset(inputs_shape_[kInputAntiquantOffset].size(), 1);
  Dimensions batch_strategy_quant_scale(inputs_shape_[kInputQuantScale].size(), 1);
  Dimensions batch_strategy_quant_offset(inputs_shape_[kInputQuantOffset].size(), 1);
  Dimensions batch_strategy_bias(inputs_shape_[kInputBias].size(), 1);
  ...
  strategy_v = {batch_strategy_x,
                batch_strategy_weight,
                batch_strategy_antiquant_scale,
                batch_strategy_antiquant_offset,
                batch_strategy_quant_scale,
                batch_strategy_quant_offset,
                batch_strategy_bias};
